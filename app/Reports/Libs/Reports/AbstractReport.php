...
abstract class AbstractReport
{
    ...
    public function __construct(Application $app, Organization $organization, $start, $end)
    {
        $this->setApp($app);
    ...
    public function output($type, $stream = false, Response $res = null)
    {
        ...
        if ($type == 'html') {
            ...
            $pubDir = $this->app['config']->get('dirs.public');
            $data = [
                'css' => 'file://'.$pubDir.'/css/report.css',
            ...
            foreach ($sections as $section) {
                ...
                $entireTable = array_merge(
                    [(array) array_value($section, 'header')],
                    (array) array_value($section, 'rows'),
                    [(array) array_value($section, 'footer')]);
