	...
	{else}
		...
		{if $awaitingApproval}
			<p class="text-center alert alert-info">
				We are currently waiting for the volunteer coordinator at {$org.name} to approve your request. When they approve your request we will send you an e-mail. If it is taking too long then we recommend reaching out to them.
		{else}
			<p class="text-center alert alert-info">
				<span>
					Please request to join in order to access the Volunteer Hub for {$org.name}. {if !$app.user->isLoggedIn()}If you are already a member, then you must be signed in to InspireVive.{/if}
				</span>
			</p>
			...
			<form method="post" action="{$orgObj->url()}/volunteers" class="form-horizontal">
				...
				<div class="form-group">
					<div class="col-md-4 col-md-offset-{if $app.user->isLoggedIn()}4{else}2{/if}">
